...
import { refusalDataQuery } from "@/sanity/query";
import { RefusalPolicyPageType } from "@/types";
...
const page = async () => {
  const returnPageData: RefusalPolicyPageType = await client.fetch(refusalDataQuery);
  ...
  return (
    <>
      ...
      >
        ...
        <div className="mt-[45px] md:mt-[30px] xl:mt-[76px] text-[#100F0F] flex flex-col md:flex-row px-[40px] md:px-[50px] gap-x-[28px] xl:gap-x-[62px] mx-auto max-w-[1050px]">
          <div className="text-[18px] leading-[147%] tracking-[0.18px]">
            <div className="font-theme-font-bold">Our Policy</div>
            <div className="font-theme-font-light">
              <PortableText components={components} value={returnPageData?.ourPolicy} />
            </div>
          </div>
        </div>
      </div>
